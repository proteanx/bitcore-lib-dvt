...
Now, this could just be serialized to hexadecimal ASCII values (`transaction.serialize()`) and sent over to the devaultd reference client.

```sh
devault-cli sendrawtransaction <serialized transaction>
```
